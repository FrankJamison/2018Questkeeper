# QuestKeeper | A D&D 5e Character Tracker

Live preview: https://questkeeper.fcjamison.com/
...
- MySQL or MariaDB
- A web server (Apache/nginx/IIS) OR PHP’s built-in dev server

VS Code:

- The task in `.vscode/tasks.json` opens the live preview in Chrome.
...
This repo supports two git-ignored config files:

- **Local development**: `includes/db.local.inc.php`
- **Deployed environments**: `includes/db.config.inc.php`

Local development (recommended):

- Copy: `includes/db.local.inc.php.example` → `includes/db.local.inc.php`
...
- On localhost, `includes/variables.inc.php` prefers `includes/db.local.inc.php`
- Otherwise it prefers `includes/db.config.inc.php`

### 3) Serve the Project

Use any PHP-capable web server (Apache, nginx+php-fpm, IIS) and point it at the project so `index.php` is reachable.

Alternatively, for quick local dev from the repo root:
...
- in a subfolder (e.g. `http://localhost/some/subfolder/`)
...
- For localhost, confirm `includes/db.local.inc.php` exists and has correct credentials.
- For deployed environments, confirm `includes/db.config.inc.php` exists and has correct credentials.
